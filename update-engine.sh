...
# Get release info - either a specific version or latest from GitHub API
get_release() {
	local target_version="${1:-}"

	if [ -n "$target_version" ]; then
		# Specific version requested - build URL directly, no API call needed
		log "Using specified version: $target_version"
		local download_url="https://github.com/${REPO_OWNER}/${REPO_NAME}/archive/refs/tags/${target_version}.tar.gz"
		log_success "Target release: $target_version"
		echo "$target_version|$download_url"
		return 0
	fi

	log "Fetching latest release information from GitHub (including pre-releases)..."
	...
	# Fetch all releases (includes pre-releases) sorted by newest first
	local releases_json
	if ! releases_json=$(eval "curl $curl_opts \"$GITHUB_API/releases?per_page=5\""); then
	...
	if ! echo "$releases_json" | jq empty 2>/dev/null; then
		...
		log "Response: $releases_json"
		exit 1
	fi

	# Pick the first (newest) release — this includes pre-releases
	local release_info
	release_info=$(echo "$releases_json" | jq '.[0]')
	...
	published_at=$(echo "$release_info" | jq -r '.published_at // .created_at')
	...
	# Build tarball URL from tag name
	download_url="https://github.com/${REPO_OWNER}/${REPO_NAME}/archive/refs/tags/${tag_name}.tar.gz"

	if [ "$is_prerelease" = "true" ]; then
		log_warning "Latest release is a pre-release: $tag_name"
	fi
	log_success "Latest release: $tag_name (published: $published_at, pre-release: $is_prerelease)"
...
download_release() {
	...
	log "Downloading release $tag_name from $download_url ..."
	...
	# Download release tarball with timeout and progress
	if ! curl -L --connect-timeout 30 --max-time 300 --retry 3 --retry-delay 5 -o "$archive_file" "$download_url" 2>/dev/null; then
		log_error "Failed to download release from GitHub (URL may be invalid - check that the tag exists)"
		rm -rf "$temp_dir"
		exit 1
	fi
	...
	log_success "Downloaded release ($(du -h "$archive_file" | cut -f1))"
	...
	# GitHub tarballs create: ProxmoxVE-Local-<tag_without_v>/
	local extracted_dir
	...
	if [ -z "$extracted_dir" ]; then
		extracted_dir=$(find "$temp_dir" -maxdepth 1 -type d ! -path "$temp_dir" 2>/dev/null | head -1)
	fi

	if [ -z "$extracted_dir" ]; then
		log_error "Could not find extracted directory"
		log "Contents of $temp_dir:"
		ls -la "$temp_dir" | while read -r line; do log "  $line"; done
		rm -rf "$temp_dir"
		exit 1
	fi

	log_success "Release extracted successfully to $(basename "$extracted_dir")"
	echo "$extracted_dir"
}
...
update_files() {
	...
	# For zip archives: ProxmoxVE-Local-<version>/
	# For tarballs: community-scripts-ProxmoxVE-Local-<hash>/
	local actual_source_dir
	actual_source_dir=$(find "$source_dir" -maxdepth 1 -type d -name "${REPO_NAME}-*" | head -1)
	...
	# If source_dir itself contains package.json, use it directly
	if [ -z "$actual_source_dir" ] && [ -f "$source_dir/package.json" ]; then
	...
	if [ -z "$actual_source_dir" ]; then
		...
		log "Contents:"
		ls -la "$source_dir" | while read -r line; do log "  $line"; done
		return 1
	fi
...
main() {
	...
	if [ -z "${PVE_UPDATE_RELOCATED:-}" ] && [ -f "package.json" ] && [ -f "server.js" ]; then
		...
		local args="--relocated"
		[ -n "$target_version" ] && args="$args $target_version"
		bash "$0" $args
		exit $?
	fi
	...
	local release_info
	release_info=$(get_release "$target_version")
