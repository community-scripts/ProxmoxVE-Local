...
export function ScriptNotesPanel({ slug }: ScriptNotesPanelProps) {
  ...
  function startEdit(note: { id: number; title: string; content: string; is_shared: boolean }) {
  ...
  function handleSave() {
    ...
    if (editingId !== null) {
      updateMutation.mutate({ id: editingId, title, content, isShared: isShared });
    } else {
  ...
  return (
    <div>
      ...
      {(isAdding || editingId !== null) && (
        <div className="border-border bg-card mb-4 space-y-3 rounded-lg border p-3">
          <input
            ...
            className="border-border bg-background text-foreground placeholder:text-muted-foreground w-full rounded-md border px-3 py-1.5 text-sm outline-none focus:ring-1 focus:ring-primary"
            maxLength={200}
          />
          <textarea
            ...
            rows={3}
            className="border-border bg-background text-foreground placeholder:text-muted-foreground w-full resize-y rounded-md border px-3 py-1.5 text-sm outline-none focus:ring-1 focus:ring-primary"
            maxLength={10000}
          />
      ...
      <div className="space-y-2">
        {notes.map((note: { id: number; title: string; content: string; is_shared: boolean; updated_at: Date }) => (
          <div
            key={note.id}
